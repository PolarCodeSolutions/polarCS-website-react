...
const DynamicPricingBox = ({
  ...
  const [selectedServices, setSelectedServices] = useState(
    services.reduce((acc, service) => ({ ...acc, [service.text]: false }), {})
  );

  const handleServiceChange = (serviceText: string) => (event: React.ChangeEvent<HTMLInputElement>) => {
    setSelectedServices({
      ...selectedServices,
      [serviceText]: event.target.checked
    });
  };

  const calculateTotalPrice = () => {
    const selectedServicePrices = services
      .filter(service => selectedServices[service.text])
      .reduce((total, service) => total + service.price, 0);
    return Math.round(basePrice + selectedServicePrices);
  };
        ...
        <div>
          {services.map((service, index) => (
            <OfferList
              ...
              text={service.text}
              price={service.price}
              isChecked={selectedServices[service.text]}
              onChange={handleServiceChange(service.text)}
              showIcons={false}
            />
          ))}
        </div>
